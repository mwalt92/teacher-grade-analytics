...
export function DashboardShell({ courseName="ACP Calculus I M211", schoolYear="2026–2027", sectionName="Semester 1", studentCount=demoStudents.length, dataMode="Demo data only" }:DashboardShellProps) {
  ...
  return <main className="app-shell">
    <header className="topbar"><div><p className="eyebrow">Teacher Grade Analytics</p><h1>{courseName}</h1><p className="subtle">{schoolYear} • {sectionName} • {period}</p><p className="subtle">Preview diagnostic build: 7f3a-test</p></div><div className="toolbar-group"><select aria-label="Select grading period" value={period} onChange={e=>setPeriod(e.target.value)}><option>Q1</option><option>Q2</option><option>S1</option></select><a className="primary-button" href="/assignments/new?diagnostic=7f3a-test"><ClipboardPlus size={18}/> New Assignment TEST</a></div></header>
    <nav className="main-nav" aria-label="Teacher navigation">{navItems.map(item=>item==="Students"?<Link key={item} className="nav-button" href="/students">{item}</Link>:<button key={item} className={activeNav===item?"nav-button active":"nav-button"} onClick={()=>setActiveNav(item)}>{item}</button>)}</nav>

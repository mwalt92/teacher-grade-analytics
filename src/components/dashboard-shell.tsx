...
export function DashboardShell({ courseName="ACP Calculus I M211", schoolYear="2026–2027", sectionName="Semester 1", studentCount=demoStudents.length, dataMode="Demo data only" }:DashboardShellProps) {
  ...
  return <main className="app-shell">
    <header className="topbar"><div><p className="eyebrow">Teacher Grade Analytics</p><h1>{courseName}</h1><p className="subtle">{schoolYear} • {sectionName} • {period}</p></div><div className="toolbar-group"><select aria-label="Select grading period" value={period} onChange={e=>setPeriod(e.target.value)}><option>Q1</option><option>Q2</option><option>S1</option></select><button type="button" className="primary-button" onClick={()=>window.location.assign("/assignments/new")}><ClipboardPlus size={18}/> New Assignment</button></div></header>

  ...
  return <div className={styles.wrap}>
    ...
    {step === 1 ? <form className={styles.form} onSubmit={review}>
      <section className="panel">
        ...
        <h2>Course basics</h2>
        <p className="subtle">Create the course and its first class period for {schoolYearLabel}. You can add more sections immediately afterward.</p>
      ...
      <section className="panel">
        ...
        <h3>Blank course or existing configuration?</h3>
        <p className="subtle">Copying configuration never copies students, rosters, assignments, scores, or grade history.</p>
        <div className={styles.sourceGrid}>
          ...
          {sources.map((source) => <button type="button" key={source.offeringId} className={basics.sourceOfferingId === source.offeringId ? styles.sourceSelected : styles.sourceCard} onClick={() => chooseSource(source.offeringId)}>
            <Copy size={20}/><span><strong>{source.label}</strong><small>{source.categoryCount} categories • {source.assignmentTypeCount} assignment types • {source.gradingPeriodCount} grading periods</small></span>
          </button>)}
        </div>
      </section>
      ...
      <section className="panel">
        ...
        <div className={styles.reviewGrid}>
          <div><span>School year</span><strong>{schoolYearLabel}</strong></div>
          ...
          <div className={styles.wide}><span>Starting point</span><strong>{selectedSource ? selectedSource.label : "Blank Course"}</strong></div>
        </div>
      </section>
      ...
      {selectedSource ? <section className="panel">
        ...
        <p className="subtle">These settings become independent copies. Changing the new course later will not change {selectedSource.label}.</p>
      ...
      <section className={`panel ${styles.safety}`}>
        ...
        <div className={styles.noCopy}><span>Students</span><span>Rosters</span><span>Assignments</span><span>Scores</span><span>Missing flags</span><span>Retakes</span><span>PowerSchool history</span></div>
      </section>

...
export default async function GradebookPage({searchParams}:GradebookPageProps){
 ...
 return <main className="app-shell"><header className="topbar"><div><p className="eyebrow">Teacher Gradebook</p><h1>{section.courseCode?`${section.courseName} ${section.courseCode}`:section.courseName}</h1><p className="subtle">{section.sectionName} • live grades from the canonical grading engine</p></div><div className="grade-audit-header-actions"><TeacherSectionSwitcher sections={sections} activeSectionId={section.sectionId} returnTo={returnTo}/><Link className="secondary-link" href="/"><ArrowLeft size={17}/> Back to Dashboard</Link><Link className="secondary-link" href={`/gradebook/assignments${selectedPeriod?`?period=${selectedPeriod.code}`:""}`}>Assignments</Link><Link className="secondary-link" href={`/gradebook/powerschool${selectedPeriod?`?period=${selectedPeriod.code}`:""}`}>PowerSchool Comparison</Link><Link className="secondary-link" href={`/gradebook/audit${selectedPeriod?`?period=${selectedPeriod.code}`:""}`}>Grade Audit</Link></div></header>
 ...
 {roster.map(student=>{const row=rowByStudentId.get(student.studentId);return <div className={styles.row} style={{gridTemplateColumns:rowTemplate}} role="row" key={student.studentId}><span className={styles.student}><strong>{student.displayName}</strong>{student.email?<small>{student.email}</small>:null}</span><strong className={styles.overall}>{formatPercent(row?.overallPercent??null)}</strong>{detailColumns.map(column=><span key={column.key}>{formatPercent(calculation?.mode==="composite"?row?.componentPercents[column.key]??null:row?.categoryPercents[column.key]??null)}</span>)}<span className={styles.flags}>{row?.missingCount?<span className="status warning-pill">{row.missingCount} missing</span>:<span className="status neutral-pill">0 missing</span>}{row?.unenteredCount?<small>{row.unenteredCount} unentered</small>:null}</span><Link className="text-button" href={`/gradebook/audit?studentId=${student.studentId}&period=${selectedPeriod?.code??""}`}>Audit →</Link></div>})}

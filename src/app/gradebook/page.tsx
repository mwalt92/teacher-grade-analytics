...
export default async function GradebookPage({searchParams}:GradebookPageProps){
 ...
 <section className="content-wrap gradebook-content"><article className="panel gradebook-controls"><div className="panel-header"><div><p className="eyebrow">Class view</p><h3>Current grades</h3></div></div><form method="get" className="gradebook-period-form"><label><span>Grading period</span><select name="period" defaultValue={selectedPeriod?.code} aria-label="Select grading period">{selectablePeriods.map(period=><option key={period.id} value={period.code}>{period.code} — {period.name}</option>)}</select></label><button className="primary-button" type="submit">View Gradebook</button></form></article>
 <section className="metric-grid gradebook-metrics" aria-label="Gradebook summary"><article className="metric-card"><span className="metric-label">Active students</span><strong>{roster.length}</strong></article><article className="metric-card"><span className="metric-label">Class average</span><strong>{formatPercent(classAverage)}</strong></article><article className="metric-card"><span className="metric-label">Missing flags</span><strong>{missingCount}</strong></article><article className="metric-card"><span className="metric-label">Unentered scores</span><strong>{unenteredCount}</strong></article></section>
 <article className="panel full-width gradebook-table-panel"><div className="panel-header"><div><p className="eyebrow">{selectedPeriod?.code??"Gradebook"}</p><h3>{mode==="semester"?"Semester grade overview":"Quarter category overview"}</h3></div><span className="subtle">{gradedRows.length} of {roster.length} students currently have a computed grade</span></div>
 <div className={`gradebook-table ${mode==="semester"?"semester":"quarter"}`} role="table" aria-label={`${selectedPeriod?.code??"Current"} class gradebook`}>
 {mode==="semester"?<div className="gradebook-row gradebook-head" role="row"><span>Student</span><span>Current {selectedPeriod?.code}</span><span>{semesterQuarterCodes[0]}</span><span>{semesterQuarterCodes[1]}</span><span>Exam</span><span>Missing</span><span>Audit</span></div>:<div className="gradebook-row gradebook-head" role="row"><span>Student</span><span>Current {selectedPeriod?.code}</span><span>Participation</span><span>Quizzes</span><span>Tests</span><span>Missing</span><span>Audit</span></div>}
 {roster.map(student=>{const row=rowByStudentId.get(student.studentId);return <div className="gradebook-row" role="row" key={student.studentId}><span className="gradebook-student"><strong>{student.displayName}</strong>{student.email?<small>{student.email}</small>:null}</span><strong className="gradebook-overall">{formatPercent(row?.overallPercent??null)}</strong>{mode==="semester"?<><span>{formatPercent(row?.componentPercents[semesterQuarterCodes[0]]??null)}</span><span>{formatPercent(row?.componentPercents[semesterQuarterCodes[1]]??null)}</span><span>{formatPercent(row?.componentPercents.EXAM??null)}</span></>:<><span>{formatPercent(row?.categoryPercents.participation??null)}</span><span>{formatPercent(row?.categoryPercents.quiz??null)}</span><span>{formatPercent(row?.categoryPercents.test??null)}</span></>}<span className="gradebook-flags">{row?.missingCount?<span className="status warning-pill">{row.missingCount} missing</span>:<span className="status neutral-pill">0 missing</span>}{row?.unenteredCount?<small>{row.unenteredCount} unentered</small>:null}</span><Link className="text-button" href={`/gradebook/audit?studentId=${student.studentId}&period=${selectedPeriod?.code??"Q1"}`}>Audit →</Link></div>})}

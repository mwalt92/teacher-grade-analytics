...
export default async function GradebookPage({searchParams}:GradebookPageProps){
 ...
 return <main className="app-shell"><header className="topbar"><div><p className="eyebrow">Teacher Gradebook</p><h1>{section.courseName}</h1><p className="subtle">{section.sectionName} • live grades from the canonical grading engine</p></div><div className="grade-audit-header-actions"><Link className="secondary-link" href={`/gradebook/assignments${selectedPeriod?`?period=${selectedPeriod.code}`:""}`}>Assignments</Link><Link className="secondary-link" href={`/gradebook/powerschool${selectedPeriod?`?period=${selectedPeriod.code}`:""}`}>PowerSchool Comparison</Link><Link className="secondary-link" href={`/gradebook/audit${selectedPeriod?`?period=${selectedPeriod.code}`:""}`}>Grade Audit</Link><Link className="secondary-link" href="/">Dashboard</Link></div></header>

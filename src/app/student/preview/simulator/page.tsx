...
type PreviewSimulatorPageProps = { searchParams: Promise<{ studentId?: string; period?: string; sectionId?: string; anchorSectionId?: string }> };
...
export default async function PreviewSimulatorPage({ searchParams }: PreviewSimulatorPageProps) {
  ...
  const simulatorParams = new URLSearchParams(navParams);
  ...
  const sectionSwitcher = <TeacherSectionSwitcher sections={sections} activeSectionId={section.sectionId} returnTo="/student/preview/simulator"/>;
  ...
  return <StudentSimulatorView
    ...
    data={data}
    actionPath="/student/preview/simulator"
    hiddenFields={[
    ...
    preview
    previewLabel="Read-only teacher preview using the selected student's real grade data and the same Grade Simulator calculation engine."
    ...
    simulatorHref={`/student/preview/simulator?${simulatorParams.toString()}`}
